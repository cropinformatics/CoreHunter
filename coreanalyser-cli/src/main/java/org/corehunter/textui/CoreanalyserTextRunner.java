...
{
	private final String[]	measureNames	= { "MR", "MRmin", "CE", "CEmin", "SH",
	    "HE", "NE", "PN", "CV"	         };
		...
		try
    {
	    ...
	    // try to create dataset
	      AccessionSSRMarkerMatrix<Integer> dataset = new AccessionSSRMarkerMatrixListImplDataFileReader(
	          new File(collectionFiles[i])).readData();
	    	...
	    	for (int j = 0; j < measureNames.length; j++)
	    	{
	    		...
	    		try
	    		{
	    			objectiveFunction.addObjectiveFunction(
	    			    MeasureFactorySSR.createMeasure(measure), 1.0);
	    		}
	    	...
	    	SubsetSolution<Integer> solution = new UnorderedIntegerListSubsetSolution(dataset.getIndices(), integerSubsetGenerator.first()) ;
	    ...
	    for (int j = 0; j < measureNames.length; j++)
	    {
	    	System.out.format("%" + (precision + 4) + "." + precision + "f ",
	    	    total[j] / collectionFiles.length);
	    }
	    System.out.println("");
    }
  ...
  private void setupOptions()
	{
		...
		opts.addOption(OptionBuilder.withArgName("decimal_count").hasArg()
	...
	private void showUsage()
	{
		System.out.println("");
		System.out
		    .println("usage: coreanalyser [options] <file1> [<file2> [<file3>...]]");
		System.out.println("");
		System.out
