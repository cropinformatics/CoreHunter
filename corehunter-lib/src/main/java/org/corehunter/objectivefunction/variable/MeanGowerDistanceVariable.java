...
public class MeanGowerDistanceVariable
    extends
    AbstractSubsetObjectiveFunction<Integer, Matrix<Integer, Object, Accession, Variable>>
{
	...
	protected void handleDataSet() throws CoreHunterException
	{
		...
		discriminateVariables = new ArrayList<Variable>(getData()
		    .getColumnHeaders().getElements());
	...
	private boolean isDiscriminateVariable(Variable variable)
	{
		switch (variable.getType())
		{
			...
			case ORDINAL:
				return isCategoricalVariableDiscriminate((CategoricalVariable) variable)
				    && isRangedVariableDiscriminate((RangedVariable) variable);
			case RATIO:
				return isRangedVariableDiscriminate(((RangedVariable) variable));
			default:
				return false;
	...
	private boolean isRangedVariableDiscriminate(RangedVariable rangedVariable)
	{
		...
		return rangedVariable.getMaximumValue().doubleValue() > rangedVariable
		    .getMinimumValue().doubleValue();
	}
	...
	public double calculate(SubsetSolution<Integer> solution)
	    throws CoreHunterException
	{
		List<Integer> subsetIndices = new ArrayList<Integer>(
		    solution.getSubsetIndices());
		...
		for (int i = 0; i < subsetIndices.size(); ++i)
		{
			for (int j = 0; j < i; ++j)
			{
				if (distanceMatrix[i][j] == null)
					distanceMatrix[i][j] = calculate(subsetIndices.get(i),
					    subsetIndices.get(j));
	...
	private double calculate(int indexA, int indexB) throws UnknownIndexException
	{
		...
		while (iterator.hasNext())
		{
			evaluation = evaluation
			    + calculate(iterator.next(), getData()
			        .getElement(indexA, columnIndex),
			        getData().getElement(indexB, columnIndex));

			++columnIndex;
		}

		evaluation = evaluation + nonDiscriminateVariableCount; // assume
																														// discriminate
																														// variables all
																														// evaluate to 1
	...
	private double calculate(Variable variable, Object elementA, Object elementB)
	{
		if (elementA != null && elementB != null)
		{
			...
			{
				case BINARY:
					return calculateBooleanVariable((Boolean) elementA,
					    (Boolean) elementB);
				case NOMINAL:
					return calculateDiscreateVariable((CategoricalVariable) variable,
					    elementA, elementB);
				case INTERVAL:
				case ORDINAL:
				case RATIO:
					return calculateRangedVariable(((RangedVariable) variable),
	...
	private double calculateRangedVariable(RangedVariable variable,
	    Number elementA, Number elementB)
	{
		return 1 - (Math.abs(elementA.doubleValue() - elementB.doubleValue()) / (variable
		    .getMaximumValue().doubleValue() - variable.getMinimumValue()
		    .doubleValue()));
	}
}
