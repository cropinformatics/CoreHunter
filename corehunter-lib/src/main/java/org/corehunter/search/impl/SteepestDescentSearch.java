 ...
 * it is better than the current solution, stop search if no improvement can be
 * made. This is also called an "iterative improvement" strategy.
 */
public class SteepestDescentSearch<IndexType, SolutionType extends SubsetSolution<IndexType>, NeighbourhoodType extends SubsetNeighbourhood<IndexType, SolutionType>>
    extends
    AbstractSubsetNeighbourhoodSearch<IndexType, SolutionType, NeighbourhoodType>
{
	...
	protected SteepestDescentSearch(
	    SteepestDescentSearch<IndexType, SolutionType, NeighbourhoodType> search)
	...
	{
		return new SteepestDescentSearch<IndexType, SolutionType, NeighbourhoodType>(
		    this);
	}
	...
	protected void runSearch() throws CoreHunterException
	{
		...
		while (canContinue())
		{
			...
			move = getNeighbourhood().performBestMove(getCurrentSolution(),
			    getObjectiveFunction(), evalution);
