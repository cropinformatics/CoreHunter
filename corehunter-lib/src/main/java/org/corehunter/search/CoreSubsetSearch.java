...
public final class CoreSubsetSearch
{
	    ...
	    double minT, double maxT, int mcSteps)
	{
		...
		for (int i = 0; i < numReplicas; i++)
		{
			...
			replicas[i] = new SimpleMonteCarloReplica(ac, pm, nh.clone(), mcSteps,
			    -1, sampleMin, sampleMax, T);
			replicas[i].init();
		}
		...
		while (cont && System.currentTimeMillis() < eTime)
		{
			...
			for (int i = 0; i < futures.size(); i++)
			{
				try
				{
					futures.get(i).get(); // doesn't return a result, but blocks until
																// done
				}
			...
			for (int i = 0; i < numReplicas; i++)
			{
				...
				if (bestRepScore > bestScore
				    || (bestRepScore == bestScore && replicas[i].getBestCore().size() < bestCore
				        .size()))
				{
					...
					System.out.println("best score: " + bestRepScore + "\tsize: "
					    + bestCore.size() + "\ttime: " + lastImprTime / 1000.0);
		...
		System.out.println("### End time: " + (System.currentTimeMillis() - sTime)
		    / 1000.0);
	 ...
	 */
	public static AccessionCollection exhaustiveSearch(AccessionCollection ac,
	    PseudoMeasure pm, int sampleMin, int sampleMax, boolean output)
	{
		...
		if (output)
			System.out.println("Nr of possible core sets: " + nr + "\n-------------");
	...
	/**
	 * Steepest Descent search. Always continue with the best of all neighbors, if
	 * it is better than the current core set, and stop search if no improvement
	 * can be made. This is also called an "iterative improvement" strategy.
	 * 
	 * @param ac
	 * @param nh
	 * @param pm
	...
	public static AccessionCollection steepestDescentSearch(
	    AccessionCollection ac, Neighborhood nh, PseudoMeasure pm, int sampleMin,
	    int sampleMax, double runtime, double minProg)
	{
	    ...
	    double runtime, double minProg, double stuckTime, int tabuListSize)
	{
		...
		while (cont && tb.getCurrentThreadCpuTime() < eTime)
		{
			...
			addIndex = nh.genBestNeighbor(core, unselected, tabuList, bestScore, pm,
			    cacheId);
			score = pm.calculate(core, cacheId);
			    ...
			    || (score == bestScore && core.size() < bestCore.size()))
			{
				...
				lastImprTime = tb.getCurrentThreadCpuTime() - sTime;
				System.out.println("best score: " + bestScore + "\tsize: "
				    + bestCore.size() + "\ttime: " + lastImprTime / 1000000000);
				// update progress writer
				if (WRITE_PROGRESS_FILE)
				{
					pw.updateScore(bestScore);
	...
	public static AccessionCollection mixedReplicaSearch(AccessionCollection ac,
	    PseudoMeasure pm, int sampleMin, int sampleMax, double runtime,
	    double minProg, double stuckTime, int nrOfTabuReplicas,
	    int nrOfNonTabuReplicas, int roundsWithoutTabu, int nrOfTabuSteps,
	    int tournamentSize, int tabuListSize, int boostNr, double boostMinProg,
	    ...
	    double maxMCTemp)
	{
		...
		// no limit on nr of steps for LR, just keeps running in background until
		// done
		...
		for (int i = 0; i < nrOfNonTabuReplicas; i++)
		{
			...
			rep = new LocalSearchReplica(ac, pm, randNh.clone(), NR_OF_LS_STEPS, -1,
			    sampleMin, sampleMax);
		...
		while (cont && System.currentTimeMillis() < eTime)
		{
			    ...
			    && cont && System.currentTimeMillis() < eTime)
			{
				...
				for (int i = 0; i < localAndMCReplicas.size(); i++)
				{
					try
					{
						...
						localAndMCReplicas.get(i).get(); // doesn't return a result, but
																						 // blocks until done
					}
					catch (InterruptedException ex)
					{
				...
				while (itr.hasNext())
				{
					...
					if (!rep.shortType().equals("Tabu"))
					{
						...
						if (rep.getBestScore() > bestScore
						    || (rep.getBestScore() == bestScore && rep.getBestCore().size() < bestCore
						        .size()))
						{
				...
				if (lrrep.isDone() && !lrChecked)
				{
					if (lrrep.getBestScore() > bestScore
					    || (lrrep.getBestScore() == bestScore && lrrep.getBestCore()
					        .size() < bestCore.size()))
					{
					...
					lrChecked = true;
					// Since LR is done, we add it to the list of replicas so that its
					// result can be used for merging
					replicas.add(lrrep);
					nrOfNonTabus++;
				}
				...
				if (!boostTimeLocked)
				{
					...
					boostTime = (boostTime * (numround - 1) + (System.currentTimeMillis()
					    - sTime - prevRoundTime) / 1000.0)
					    / numround;
					boostTime = boostTime * boostTimeFactor;
				...
				if (impr && prog < boostMinProg)
				{
					...
					// only boost with some fraction of the normal nr of boost replicas in
					// case of min prog boost
					int progBoostNr = boostNr / PROG_BOOST_FACTOR;
					boostReplicas(replicas, progBoostNr, ac, pm, randNh, NR_OF_LS_STEPS,
					    sampleMin, sampleMax);
				...
				// check boost time -- do not boost if previous boost effect still
				// visible!
				if ((System.currentTimeMillis() - sTime - Math.max(lastImprTime,
				    lastBoostTime)) / 1000.0 > Math.max(boostTime, minBoostTime)
				    && replicas.size() == nrOfNonTabuReplicas + nrOfTabuReplicas)
				{

					lastBoostTime = System.currentTimeMillis() - sTime;
					boostReplicas(replicas, boostNr, ac, pm, randNh, NR_OF_LS_STEPS,
					    sampleMin, sampleMax);
				...
				{
					// Select parents from non-tabu replicas only! (tabus are still being
					// manipulated, so skip these)
					selectParents(replicas, parents, 2 * nonTabuChildren, tournamentSize,
					    rg, "Tabu");
					// Create new children by merging parents
					createNewChildren(parents, children, rg);
					// Create new MC recplicas which use merged children as initial
					// solutions
					for (List<Accession> child : children)
					{
						// New REMC replicas
						Replica rep = new SimpleMonteCarloReplica(ac, pm, randNh.clone(),
						    NR_OF_LS_STEPS, -1, sampleMin, sampleMax, minMCTemp
						        + rg.nextDouble() * (maxMCTemp - minMCTemp));
						nrOfNonTabus++;
			...
			{
				// Tabu replicas have finished --> check for improvements & count stuck
				// tabus
				nrStuck = 0;
				Iterator<Replica> itr = replicas.iterator();
				while (itr.hasNext())
				...
				if (tabuChildren > 0)
				{
					...
					for (List<Accession> child : children)
					{
						...
						Replica rep = new TabuReplica(ac, pm, heurNh.clone(),
						    nrOfTabuSteps, -1, sampleMin, sampleMax, listsize);
						nrOfTabus++;
			...
			{
				// Tabu replicas have not finished, which means search was stopped
				// during inner loop
				// of non-tabu replicas. Search will stop, so don't do anything anymore
				// at this point.
			}
	    ...
	    int nrOfLsSteps, int sampleMin, int sampleMax)
	{
		...
		for (int i = 0; i < boost; i++)
		{
			Replica rep;
			// create LS Replica
			rep = new LocalSearchReplica(ac, pm, randNh.clone(), nrOfLsSteps, -1,
			    sampleMin, sampleMax);
			rep.init();
			replicas.add(rep);
		}
	...
	public static AccessionCollection semiForwardSelection(
	    AccessionCollection ac, PseudoMeasure pm, int sampleMin, int sampleMax)
	{
