...
public final class SSRDataset extends AccessionDataset<List<Double>>
{
	...
	public SSRDataset(Collection<String> accessions,
	    Map<String, List<String>> markersToAlleles)
	{
		...
		// create an alias for traitName/traitIndex/traitCount for consistent naming
		// within this class
	...
	public static SSRDataset createFromlist(List<String[]> lines)
	{
	...
	public static SSRDataset createFromArray(String[][] lines)
	{
		...
		String nextLine[];
			...
			for (int lineNumber = 1 ; lineNumber < lines.length ; ++lineNumber)
			{
				...
				{
					System.err.println("Dataset is not properly formatted on line "
					    + lineNumber);
					System.err
					    .println("Please refer to the CoreHunter manual.  "
				...
				if (nextLine[0].equalsIgnoreCase("DIST"))
				{
					...
					for (int i = 2; i < nextLine.length; i++)
					{
						...
						catch (NumberFormatException nfe)
						{
							...
							System.err.println("");
							System.err
							    .println("Invalid external distance value for accession '"
							        + accession);
							return null;
						}
					}
					...
					for (int i = 2; i < nextLine.length; i++)
					{
						...
						catch (NumberFormatException nfe)
						{
							...
							System.err.println("Invalid allele value for accession '"
							    + accession + "' marker '" + marker + "' allele '" + allele
							    + "'");
							return null;
						}
	 ...
	 * within a marker (for a particular accession) or, the sum of all values is
	 * 1.0.
	 * 
	 * @param accession
	 *          the accession index
	 ...
	 *          the marker index
	 * @param alleleValues
	 *          new allele values for marker at accession
	 * @throws UnknownAccessionException
	 *           when accession is not found in the data set
	 * @throws UnknownTraitException
	 *           when marker / allele is not found in the data set
	 */
	public void setValue(String accession, String marker, String allele,
	    Double alleleValue) throws UnknownAccessionException,
	    UnknownTraitException
	{
		Integer acIndex = accessionIndex.get(accession);
		if (acIndex == null)
			throw new UnknownAccessionException("No accession found with id: "
			    + accession);
		Integer mIndex = markerIndex.get(marker);
		if (mIndex == null)
			throw new UnknownTraitException("No marker found with id: " + marker);
		Integer alIndex = alleleIndex.get(mIndex).get(allele);
		if (alIndex == null)
			throw new UnknownTraitException("No allele found with id: " + allele);
	 ...
	 *          new allele values for marker at accession
	 * @throws UnknownAccessionException
	 *           when accession is not found in the accession list
	 * @throws UnknownTraitException
	 *           when marker is not found in the marker list
	 */
	public void setValue(String accession, String marker,
	    List<Double> alleleValues) throws UnknownAccessionException,
	    UnknownTraitException
	{
		Integer acIndex = accessionIndex.get(accession);
		if (acIndex == null)
		...
		{
			throw new UnknownTraitException("Marker " + marker + " requires "
			    + alleleName.get(mIndex).size() + " allele values.  "
			    + alleleValues.size() + " were provided.");
		}
	...
	private void setValue(int accession, int marker, int allele,
	    Double alleleValue)
	{
